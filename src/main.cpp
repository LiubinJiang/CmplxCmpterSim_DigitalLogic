...
void display_statements(std::ostream &out, const evl_statements &statements){
    ...
    for (evl_statements::const_iterator iter=statements.begin();iter!=statements.end();++iter,++count){
        if((*iter).type==evl_statement::ENDMODULE){
            std::cout<<"Statement "<<count;
            std::cout<<": ENDMODULE" <<std::endl;
        }
        if((*iter).type==evl_statement::MODULE){
            std::cout<<"Statement "<<count;
            std::cout<<": MODULE" <<statements.end()-statements.begin()<<std::endl;
        }
    }*/
    ...
    /*for(int count=0;count<statements.size();count++){
        for(int i=0;i<statements[count].tokens.size();i++){
            std::cerr<<statements[count].tokens[i].str <<std::endl;
        }
    }*/
}
...
bool store_tokens_to_file(std::string file_name,const std::list<evl_statement> statements){
    ...
    for(evl_statements::const_iterator iter=statements.begin();iter!=statements.end();++iter){
        ...
        if((*iter).type==evl_statement::MODULE){
            ...
            //for(evl_tokens::const_iterator i=statement.tokens.begin();i!=statement.tokens.back();++i){
            output_file<<"module "<<module_tokens.front().str<<std::endl;
            //}
            //output_file<<"module "<<(*iter).tokens.front().str<<std::endl;
            }
...
int main(int argc, const char * argv[])
{
    ...
    store_tokens_to_file(std::string(argv[1])+".syntax", statements);
    return 0;
}
